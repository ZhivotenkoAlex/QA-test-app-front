...
function UsefullInfo({ books, resources }) { 
    return (
      <section className={styles.materials}>
        ...
        {books.map(({ id, name }) => (
        <li className={styles.item} key={id}><UsefullInfoItemLiter id={id} name={name}/></li>
        ))}
        </ol>
        ...
        <li className={styles.item} key={id}><UsefullInfoItemResours id={id} name={name} url={url}/></li>
        ))}
        </ol>
        </section>
      );
